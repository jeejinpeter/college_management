...
class Truststaff extends CI_Controller {
...
public function index()
    { 
     if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    { 
      $this->load->helper(array('form', 'url'));
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/dashboard');
    }else
    {
    ...
    public function display_non_teaching_list()
  {
      ...
      if($data){
        $this->load->view('Truststaff/header'); 
        $this->load->view('Truststaff/non_teaching_staff_list',$data);
      }
      else {
      $this->session->set_flashdata('msg','<div class="alert alert-danger text-center"> Error Occured.  Please try again later!!!</div>');
      redirect('Truststaff/index');
  ...
  public function display_all_staff_list()
  {
      ...
      if($data){
        $this->load->view('Truststaff/header'); 
        $this->load->view('Truststaff/all_staff_list',$data);
      }
      else {
      $this->session->set_flashdata('msg','<div class="alert alert-danger text-center"> Error Occured.  Please try again later!!!</div>');
      redirect('Truststaff/index');
  ...
  public function link_add_admin()
      ...
      if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
      ...
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/admin_detail_view', $data);
    }else
   {
       redirect('Superadmin/index','refresh'); 
    }
...
$res = $this->User_model->create_admin($data);
  if($res == true){ 
    $this->session->set_flashdata('msg','<div class="alert alert-success text-center"> Successfully Added new Admin. </div>');
         redirect('Truststaff/link_add_admin');
          }
          else
            { 
         ...
         if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
      ...
      if($data){ 
   echo form_open("Truststaff/update_admin_details"); 
    ...
    if($this->Admin_model->update_admin($data, $id))
     {
      ...
      redirect('Truststaff/link_add_admin', 'refresh'); 
     }else {
      $this->session->set_flashdata('msg','<div class="alert alert-danger text-center"> Error Occured.  Please try again later!!!</div>');
      redirect('Truststaff/link_add_admin');
 }  
   ...
   public function change_staff_password()
    {
      if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')
        { 
              ...
              $this->load->helper(array('form', 'url'));
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/change_password_view', $data);
        }
      else{
 ...
 public function list_all_Admission()
  ...
  if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')
    ...
    if($data){    
         $this->load->view('Truststaff/header');   
         $this->load->view('Truststaff/list_all_admission',$data);          
            }else{
  ...
  if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')
    ...
    if($data){    
         $this->load->view('Truststaff/header');   
         $this->load->view('Truststaff/list_admission',$data);
 ...
 public function get_by_search()
    ...
    if($data){ 
         $this->load->view('Truststaff/header');   
         $this->load->view('Truststaff/search',$data);        
        }else{
            $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Please Try again!!!</div>');
            redirect('Truststaff/index', 'refresh');
          }
...
public function Add_fund_view()
      ...
      if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
      $this->load->helper(array('form', 'url'));
     $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/add_fund');
    }
  else
    {
       redirect('Superadmin/index','refresh'); 
      ...
      if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
     ...
     $result = $this->Admin_model->Add_fund($data); 
          if($result){     
               ...
               redirect('Truststaff/Add_fund_view');
               }
          else
         {
...
public function list_college_fund()
  ...
  if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')
    ...
    if($data){
    $this->load->view('Truststaff/header');   
    $this->load->view('Truststaff/list_fund',$data);
         }else{
            $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Please Try again!!!</div>');
            redirect('Truststaff/index', 'refresh');
          }
...
public function edit_funds($id)
    ...
    {      
     ...
     if($id != NULL){
           $this->load->view('Truststaff/header');
           $this->load->view('Truststaff/update_fund',$data);   
    }
    else{
            $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Please Try again!!!</div>');
            redirect('Truststaff/index', 'refresh');
    ...
    $id = $this->input->post('id');
      $data = array(
       ...
       $result = $this->Admin_model->update_fund($id,$data); 
          if($result){     
                $this->session->set_flashdata('msg','<div class="alert alert-success text-center">You have Successfully Updated!</div>');
               redirect('Truststaff/list_college_fund');
               }
          else
         {
          $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Something went wrong!!!</div>');
 ...
 public function Delete_fund()
  ...
  $result = $this->Admin_model->delete_fund($id);
   if($result)
    ...
    {
      $this->session->set_flashdata('msg','<div class="alert alert-danger text-center">Something went wrong!!!</div>');
       redirect('Truststaff/list_college_fund','refresh'); 
    }
    }
   else
    {
 ...
 /*.................Fee Remittance.....................*/
   public function get_fees_view()
         ...
         if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')   
    {
       ...
       $data['batch']=$this->Batch_model->get_all_batch();
       $this->load->view('Truststaff/fees_remit_view.php',$data); 
    }else
    {
       redirect('Superadmin/index','refresh'); 
    }
  ...
  public function view_fees_processing()
         ...
         if(!empty($course_fees))
    {
      ...
      $data['total_amount'] = 0; 
          ...
          $data['students']=$this->Course_model->get_class_section_id_batch($course, $sem,$batch); 
          $this->load->view('Truststaff/header');
          $this->load->view('Truststaff/fees_remit_stu_view.php',$data);  
        }
  else
    {
       redirect('Superadmin/index','refresh'); 
         ...
         if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
       ...
       $data['val']=$this->Subject_model->get_semester();
      $data['batch']=$this->Batch_model->get_all_batch();
       $this->load->view('Truststaff/fees_strc_view.php',$data);  
    }else
    {
       redirect('Superadmin/index','refresh'); 
    }
      ...
      if ($this->form_validation->run() == FALSE)
       ...
       $this->load->view('Truststaff/fees_remit_view.php',$data);
        ...
        $data['students']=$this->Course_model->get_all_studentfees($course,$sem,$batch); 
          $this->load->view('Truststaff/header');
          $this->load->view('Truststaff/fees_strclist_view.php',$data); 
        }
  }else
    {
       redirect('Superadmin/index','refresh'); 
...
public function generate_all_fees_payment_pdf()
{
    ...
    $pdf->AddPage();
          ...
          $batch= $data['batch_id'];
          $data['students']=$this->Course_model->get_all_studentfees($course,$sem,$batch); 
    $html = $this->load->view('Truststaff/fees_all_student_report',$data,true);
  ...
  public function link_send_message()
    ...
    {
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/send_message_view'); 
    }
  else
    {
       redirect('Superadmin/index','refresh'); 
 ...
 public function get_sent_messages()
      ...
      if(isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
      ...
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/send_messages_list', $data); 
    }
  else
    {
       redirect('Superadmin/index','refresh'); 
...
/*...........................Expenses.........................*/
  public function get_college_expenses()
    ...
    {
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/get_dept_expense_view'); 
    }
  else
    {
       redirect('Superadmin/index','refresh'); 
  ...
  public function get_month_expense()
      ...
      if(isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true && $_SESSION['role'] === '8' && $_SESSION['role_des'] === 'superstaff')  
    {
      ...
      $this->load->view('Truststaff/header');
      $this->load->view('Truststaff/get_dept_expense_view',$data); 
    }
  else
    {
       redirect('Superadmin/index','refresh'); 
  ...
  public function generate_expenses_pdf($m)
{
    ...
    $html = $this->load->view('Truststaff/college_expense_report',$data,true);
    $pdf->writeHTML($html, true, false, true, false, '');
    ob_end_clean();     
    $pdf->Output();
 }   
